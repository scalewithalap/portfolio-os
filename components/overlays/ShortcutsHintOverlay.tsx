...
export default function ShortcutsHintOverlay() {
  ...
  const { openSpotlight, systemTheme, openApps } = useEcosystemStore();
  const isLight = systemTheme === "light";

  const hasMaximizedWindow = openApps.some(
    (app) => app.isOpen && !app.isMinimized && app.isMaximized,
  );
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Avoid capturing shortcuts when typing inside form inputs
      ...
      // ⌘ / or Ctrl / -> Toggle Shortcuts HUD
      if (isCmdOrCtrl && (key === "/" || e.key === "?")) {
        e.preventDefault();
        setIsOpen((prev) => !prev);
        return;
      }

      // ⌘ K or Ctrl K -> Toggle Spotlight Search
      ...
      // ⌘ Shift S or Ctrl Shift S -> Toggle Desktop Stacks
      if (isCmdOrCtrl && e.shiftKey && key === "s") {
        e.preventDefault();
        useEcosystemStore.getState().toggleStacks();
        return;
      }

      // ⌘ W or Ctrl W -> Close Focused Window
      if (isCmdOrCtrl && key === "w" && !e.shiftKey) {
        if (!isInput) {
          const { focusedAppId, closeApp } = useEcosystemStore.getState();
          if (focusedAppId) {
            e.preventDefault();
      ...
      if (isCmdOrCtrl && key === "m" && !e.shiftKey) {
        if (!isInput) {
          const { focusedAppId, minimizeApp } = useEcosystemStore.getState();
          if (focusedAppId) {
            e.preventDefault();
  ...
  const SHORTCUT_ITEMS = [
    ...
    {
      keys: ["⌘", "/"],
      description: "Toggle system shortcuts hint modal",
      action: () => setIsOpen((p) => !p),
      icon: Keyboard,
    },
    {
      keys: ["⌘", "W"],
      description: "Close active focused window",
      action: () => {
        const { focusedAppId, closeApp } = useEcosystemStore.getState();
        if (focusedAppId) {
          closeApp(focusedAppId);
    ...
    {
      keys: ["⌘", "M"],
      description: "Minimize active focused window",
      action: () => {
        const { focusedAppId, minimizeApp } = useEcosystemStore.getState();
        if (focusedAppId) {
          minimizeApp(focusedAppId);
    ...
    {
      keys: ["⌘", "⇧", "S"],
      description: "Toggle desktop icon stacks",
      action: () => {
        useEcosystemStore.getState().toggleStacks();
      },
      icon: Layers,
    ...
    {
      keys: ["Click", "Window"],
      description: "Bring inactive window immediately to front",
      icon: MousePointer,
    },
    {
      keys: ["Drag", "Titlebar"],
      description: "Move window frame across desktop viewport",
      icon: Move,
    },
    {
    ...
    <>
      ...
      >
        <button
          ...
          title="Press Cmd + / or Ctrl + / to toggle shortcuts"
        >
          <div
      ...
      {isOpen && (
        ...
        >
          ...
          >
            {/* Header */}
            <div
              className={`flex items-center justify-between pb-4 border-b mb-4 ${isLight ? "border-slate-200" : "border-white/10"}`}
            >
            ...
            <div className="space-y-1.5">
              {SHORTCUT_ITEMS.map((item, itemIdx) => {
            ...
            >
              <span className="flex items-center space-x-1">
                ...
                <span>anytime to toggle this box</span>
              </span>
            </div>
          </div>
        </div>
